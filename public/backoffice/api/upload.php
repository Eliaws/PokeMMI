...
log_message("REQUEST_URI: " . ($_SERVER['REQUEST_URI'] ?? 'N/A'));
log_message("QUERY_STRING: " . ($_SERVER['QUERY_STRING'] ?? 'N/A'));
log_message("SCRIPT_NAME: " . ($_SERVER['SCRIPT_NAME'] ?? 'N/A'));
log_message("REMOTE_ADDR: " . ($_SERVER['REMOTE_ADDR'] ?? 'N/A'));
log_message("HTTP_USER_AGENT: " . ($_SERVER['HTTP_USER_AGENT'] ?? 'N/A'));
log_message("HTTP_REFERER: " . ($_SERVER['HTTP_REFERER'] ?? 'N/A'));
log_message("Request method: " . ($_SERVER['REQUEST_METHOD'] ?? 'N/A'));
...
if (isset($_FILES['jaquette']) && isset($_POST['version'])) {
    ...
    // Check if $game_versions is defined (should now be from config_utils.php)
    if (!isset($GLOBALS['game_versions']) || !is_array($GLOBALS['game_versions'])) {
        $error_message = "Game versions array (map) is not defined or not an array in PHP. Check config_utils.php.";
        log_message($error_message);
        echo json_encode(["success" => false, "message" => $error_message]);
        $conn->close();
    ...
    // Validate version against the predefined list (keys)
    ...
    // Get the full game name from the map
    $game_version_name = $GLOBALS['game_versions'][$game_version_key] ?? 'Unknown Version';
